...
//import react-date-range library
...
function Header({ placeholder }) {
  ...
  //functions: for handling select date range
  const handleSelect = (ranges) => {
    setStartDate(ranges.selection.startDate);
    setEndDate(ranges.selection.endDate);
  };

  //functions: for reseting input when hit "Cancel"
  const resetInput = () => {
    setSearchInput("");
  };

  //function: search, to push search input to the result URL
  const search = () => {
    router.push({
      pathname: "/search",
      query: {
